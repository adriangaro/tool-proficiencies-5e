import { i18n, MODULE_NAME, isModuleActive, getChatData } from "../utils.js";

export const tidy5eToolCard = function (html, actor) {
    // show/hide grid layout item info card on mouse enter/leave
      ...
      let cardTrigger = itemCardsForAllItems ? html.find('.inventory-list:not(.character-actions-dnd5e) .item-list .tool-data') : html.find('.grid-layout .item-list .tool-data');
      ...
      function showCard(event){
        getBounds();
        event.preventDefault();
        let li = $(event.currentTarget).closest('.tool-data'),
            chatData = getChatData(actor, li.data("tool-id")),
            itemDescription = chatData.description,
